...
do_stage2(int orders[], int rfdmmm[], double delivx[], double delivy[], int time[], int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        currDrone->dep = rfdmmm[i];
        currDrone->del = (rfdmmm[i]+time[i]);
        currDrone->rts = (rfdmmm[i]+(time[i]*2)+ RECOVERY_TIME);    
        currDrone->wasUsed = 1;

        printf("S2, order %d, rfd %02d:%02d, drone %c, dep %02d:%02d, del %02d:%02d, rts %02d:%02d\n",
        orders[i], hour(rfdmmm[i]), minute(rfdmmm[i]),
        currDrone->label, 
        hour(currDrone->dep), minute(currDrone->dep), 
        hour(currDrone->del), minute(currDrone->del),
        hour(currDrone->rts), 
        minute(currDrone->rts));      
    }
    /*int count = 0;
    for (int j = 0; j < 26; j++) {
        if (drones[j].wasUsed) 
    ...
    printf("A Total of %d drones were was used\n", count); 
}
