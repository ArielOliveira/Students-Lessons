...
/*void
do_stage2(int orders[], int rfdmmm[], double delivx[], double delivy[], int n) {
    int rts[MAX_DRONES];
    int n_drones=0;
    int i;
    
    for (i = 0; i < n; i++){
        printf("S2, orders %d, rfd %02d:%02d, dep %02d:%02d",orders[i], 
        rfdmmm[i]/MIN_PER_HOUR, rfdmmm[i]%MIN_PER_HOUR, rfdmmm[i]/MIN_PER_HOUR, 
        rfdmmm[i]%MIN_PER_HOUR);
    } 
}*/

int
flight_time(double delivx[],double delivy[], int indx) {
    distance = sqrt((delivx[indx] * delivx[indx])+(delivy[indx] * delivy[indx]));
    time[indx] = ceil(distance[indx]/(CNST_SPEED * MIN_PER_HOUR));
    return time;
}
