...
private:
  // TODO: fields
  std::vector< std::pair<DataType, TTrie<DataType>*> > nodes;
  bool endPoint;

  // TODO: helper functions
  
  std::string as_string(std::string& s, std::vector<DataType> path) const;
  std::string elementToString(const DataType& value) const;
  static bool compareElement(const std::pair<DataType, TTrie<DataType>*>& v1, const std::pair<DataType, TTrie<DataType>*>& v2);
  typename std::vector< std::pair<DataType, TTrie<DataType>*> >::iterator findElement(const DataType& value);
};
